...
def add_rpl_mode(parser, do_both=True, add_file=True):
    ...
    parser.add_option("--rpl", "--replication", dest="rpl_mode",
                      ...
                      "using the source server as the main, "
                      "'subordinate' = include the CHANGE MASTER command for "
                      "the source server's main (only works if the source "
                      "server is a subordinate){0}.".format(rpl_mode_both),
...
def add_discover_subordinates_option(parser):
    """Add the --discover-subordinates-login option.

    This method adds the --discover-subordinates-login option that is used to
    discover the list of subordinates associated to the specified login (user and
    password).
...
def add_main_option(parser):
    """Add the --main option.

    This method adds the --main option that is used to specify the connection
    string for the server with the main role.
...
def add_subordinates_option(parser):
    """Add the --subordinates option.

    This method adds the --subordinates option that is used to specify a list of
    subordinates, more precisely their connection strings (separated by comma).
